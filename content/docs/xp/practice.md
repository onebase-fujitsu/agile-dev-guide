...
テスト駆動開発(Test-Driven Development、以下TDD)についてはテストの章で詳しく書きます。
ここでは概要だけ。

テスト駆動開発ではまず実装しようとする機能の振る舞いをユニットテストとして実装します。
ユニットテストを実装した段階では機能はまだ実装されていないため、テストは失敗します。
そして、失敗したテストをパスさせる目的で機能を実装するという流れで開発していきます。
...
ユニットテストがリグレッションテストの役割を担い、変更によって思わぬ障害が起きていないか確認できるようになります。
